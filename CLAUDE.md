...
- **`load(metadataPath, pointBudget?)`**: Lazy-imports `potree-core`, calls `potree.loadPointCloud()` with a `requestManager` built from the `FileSourceAdapter`. Detects RGB attribute in `metadata.json` and sets appropriate `PointColorType`. Sets `outputColorEncoding=1` (see Color Rendering section). Fits camera to world box. **Budget**: when `pointBudget` is omitted, it is **derived from the cloud's total point count** via `calcOptimalBudget(metadata.points)` — no fixed cap. The applied value is exposed as `loader.appliedBudget`; the Viewport syncs it back into React state (`setPointBudget`) after load so sliders/status show the real number. An explicit `config.pointBudget` passes through unchanged.
...
- **`static calcOptimalBudget(totalPoints)`**: Heuristic budget (30% / 15% / 8% of total depending on size), floored at 500K, **never above the cloud's own total — no fixed upper cap**. The three budget sliders (Settings panel, toolbar, workstation palette) scale their `max` to `metadata.points` via the shared `budgetSliderRange()` helper in viewer `lib/utils`.
...
- **`addBox(box, name?)`**: Adds a `ClipBoxEntry`, creates a `Box3Helper` outline in the scene (black while deselected), applies clipping to all point clouds. Returns the entry. A selected box shows **all handle sets simultaneously** with **no mode switching**: (1) **scaling** = 6 face-mounted **axis arrows** (`FaceHandleController`, shaft+cone per face, invisible grab-sphere hitbox, size proportional to the box diagonal — no upper clamp, a fixed cap made them invisible on site-scale boxes); (2) **rotation** = 3 **quarter-circle arcs** (`RotationRingController`, custom — NOT the stock TransformControls rotate gizmo, whose invisible picker tori span nearly the full circle and constantly stole drags; here the hitbox is a fat torus over EXACTLY the visible quarter, arcs meet Blender-style at the +X/+Y/+Z corner, radius ≈0.42 of the max box dimension — the mid-region the translate plane quads used to occupy); (3) **translate** = a single compact `TransformControls` arrow gizmo at the center, **with its two-axis plane quads (`XY`/`YZ`/`XZ`) removed** — `_removePlaneHandles()` strips them from BOTH the gizmo and picker trees (hiding is futile: the gizmo's `updateMatrixWorld` forces `visible=true` on every handle each frame; removal is the only reliable way). Pointer priority in Viewport: face arrows → rotation arcs (skipped when `isGizmoDragging()`) → translate gizmo. Orientation is stored as a full `THREE.Quaternion` on the entry (`ClipBoxEntry.quaternion`). `setTransformMode`/`getTransformMode` are **deprecated no-ops** kept for API compatibility.
...
- **`selectBox(id)`**: Lazy-initialises the translate `TransformControls` (`tcMove`), the `FaceHandleController`, the `RotationRingController` and an invisible pivot mesh, then `_showSelectionHandles()` attaches all three handle sets together. The clip toolbar has no mode buttons — only "Reset rotation". Disables OrbitControls while dragging. Selecting a box brightens its outline (yellow); deselecting returns it to black. **three r170**: `TransformControls` extends `Controls`/`EventDispatcher` (not `Object3D`), so the gizmo is added to the scene via `tc.getHelper()` (the internal `_root` Object3D), never `scene.add(tc)`. The `_raiseGizmo()` helper (forces the gizmo to render over the dense cloud via `depthTest=false` + high `renderOrder`) traverses `tc.getHelper()`, not the controls object — the controls object has no `traverse`.
...
- **`dispose()`**: Disposes the translate gizmo, face-arrow and rotation-ring controllers, and scene objects.
...
- **`finish()`**: Computes value (distance, height, area via shoelace, angle, volume via bounding box), builds permanent 3D objects (vertex dots + lines + label sprite), stores in map, fires `onChange`.
- **Rendering**: vertex markers are **constant screen-size dot sprites** (`sizeAttenuation:false`, white disc + dark ring tinted per measurement — not world-sized balls), and value labels are **constant screen-size cards** (dark card, **monospaced** white value text, measurement color as border — no accent bar; **canvas width fits the text** via `measureText` + padding, sprite width follows the canvas aspect so on-screen height stays constant). Labels anchor **on** the measured geometry: distance/height at the drawn segment's midpoint, angle at the apex, point/area/volume at the centroid/top with a screen-space lift via `sprite.center` so they don't cover their vertex dot. `measurementSphereRadius` and `measurementLabelScale` act as size multipliers. The shared dot texture must NOT be disposed per-measurement (`_disposeObjects` guards it). **Canvas gotchas**: `ctx.roundRect` accumulates into the current path — every rounded rect needs its own `ctx.beginPath()`; and resizing a canvas resets ALL context state (set the font again after sizing).
- **Volume flow**: the footprint's base plane comes from a real **cloud pick** at drag start (`pickVisiblePoint`; falls back to the target plane), the phase-1 preview is a **thin slab** on that plane (5% of the footprint diagonal, min 0.5 m — not a full-height tower), the height phase grows the box **from the base plane** (up or down with the mouse) at a **view-scaled sensitivity** (world-units-per-pixel from camera distance/FOV, like OrbitControls panning), and `setVolumeDraft` shows a **live `L × W × H m · V m³` label** during both phases.
...
### Measurement snap is throttled (no magnifier loupe), clicks are WYSIWYG
While a point-snapping measurement tool is active, the Viewport shows a 3D crosshair at the snapped point **and keeps the OS crosshair cursor visible** — hiding the cursor (`cursor: none`, tried once) left users feeling they'd lost pointer control whenever the pick lagged or missed. The OS cursor shows where the mouse IS; the snap crosshair shows where the pick will LAND. The snap uses potree's **GPU pick** (`SceneManager.pickPoint`, `pickWindowSize: 31` — generous so thin structures are easy to hit), which stalls the pipeline — so it's **throttled to one pick per animation frame** (`requestAnimationFrame`, storing the latest cursor NDC) instead of picking on every `mousemove`; per-event picking made dense clouds unusable. **Clicks commit the last previewed point** (`lastSnapRef`) rather than re-picking, so the placed point is exactly what the crosshair showed; the ref is cleared on tool switch and mouse-leave so a stale preview can't be committed. *(A cursor-following 2D-loupe magnifier was tried and removed — it showed no points and was slow; that's why the renderer no longer needs `preserveDrawingBuffer`.)*

### Measurement tools auto-open the measurements panel
Activating any `measure-*` tool switches the sidebar to the **Measurements** tab (effect in `Sidebar`) and opens the sidebar on desktop (effect in `WorkspaceLayout`, skipped on mobile where the overlay sidebar would cover the viewport).
...
Every slider in the panel has an **editable value box** beside it (`ValueBox`, keeps local text state while focused so mid-edit input isn't clobbered, commits clamped to `[min,max]`); `scale`/`unit` let a box show a friendlier number (budget in millions "M", elevation "m", multipliers "×"). Default **point size is 1.0** (smaller → crisper; was 1.5) both in the loader material and the provider state.

**DisplaySettings presets are honest about "custom"**: any manual slider tweak sets `displaySettings.preset = "custom"` (type widened to `DisplayPreset | "custom"`), so no preset card claims to be active while values have diverged — re-clicking a stale-highlighted preset used to silently reset all custom values. `measurementLineWidth` is deprecated (WebGL ignores `LineBasicMaterial.linewidth`; its slider was removed). `DisplaySettingsDialog` still exists for custom UIs, but the default layout reaches these controls through the unified Settings panel.
...
`AxisGizmo` wraps three.js's native **`ViewHelper`** (`three/examples/jsm/helpers/ViewHelper.js`) for the world-orientation gizmo — the colored X/Y/Z balls + labels, **bottom-right** (ViewHelper's native corner; the minimap moved to bottom-left to free it). `render()` just calls the native `ViewHelper.render` — no positioning wrapper. **One deliberate deviation** because the scene is **Z-up with one OrbitControls**: `ViewHelper.handleClick()` animates the camera with hard-coded **Y-up** targets and never touches OrbitControls — which would reintroduce the axis-shift bug — so `AxisGizmo.handleClick()` does its own hit-test against the gizmo axes (bottom-right dim×dim square, raycast against a replica ortho camera whose `matrixWorld` is updated once since we never render through it) and invokes `onAxisSelect(dir)`, which the Viewport flies via `CameraAnimator` with `up=(0,0,1)` (top/bottom nudged off the ±Z pole like the view presets). Registered as a post-render callback; the Viewport calls `handleClick(clientX, clientY)` first in `handleClick` and consumes the click when an axis is hit. Because the gizmo sits bottom-right (under the right sidebar), **`WorkspaceLayout` raises the sidebar's `md:` bottom to `bottom-36`** so the sidebar clears the 128px gizmo corner. Replaced the old custom `AxisWidget` (flat XYZ arrows).
