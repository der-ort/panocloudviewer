...
Tile textures use `colorSpace = LinearSRGBColorSpace` (pass-through, matching potree's sRGB output) on a plane at the cloud's min-Z. The viewport builds it after load when `config.basemap?.crs` **or** `.georeference` is set (async — proj4 is lazy-loaded), flips `basemapAvailable`, and the Layers toggle drives `setVisible`.
