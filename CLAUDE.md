...
View-layer visibility (panoramas, measurements, minimap, and the map basemap) is consolidated into the sidebar's **"Layers"** tab (`components/sidebar/layers-panel.tsx`, the first tab). **Classification** is folded into this panel as a collapsible section (its standalone tab was removed). Provider state: `showMarkers` (panoramas), `showMeasurements` (wired to `MeasurementManager.setVisible`), `showMinimap`, `showBasemap`, and `basemapAvailable` (set true once the viewport builds a basemap). The **Map basemap** row is disabled unless `basemapAvailable`. The **minimal** layout (no sidebar) keeps its layer toggles in `MinimalSettingsPopover`.
...
### Georeference detection + map basemap (`TileBasemapManager`)
`PointCloudLoader` reads `metadata.json`'s `projection` and exposes `projection`, `isGeoreferenced`, `getGeoInfo()`; the **About dialog** shows the georeference status + CRS. **Most exports are NOT georeferenced** — E57/LAS commonly drop the CRS (empty `coordinateMetadata` / zero LAS VLRs), so `metadata.json` `projection` comes out `""`. For those, supply a **manual georeference** via the `basemap` prop:
```tsx
<PanoCloudViewer source={src} basemap={{ georeference: { lat: 49.67119, lon: 10.07846, rotationDeg: 0 } }} />
```
`TileBasemapManager` (`core/tile-basemap-manager.ts`) lays **Carto Voyager raster XYZ tiles** (default; `tileUrl` overridable, attribution shown) and has **two modes**:

- **`basemap.crs` (projected mode)** — for a cloud whose rendered coords are a projected CRS. potree-core sets the octree `position = metadata.offset`, so a cloud converted from a **georeferenced LAS renders at its true projected coordinates** (large offset). NavVis/PotreeConverter often still leave `projection: ""`, so you pass the CRS yourself — a proj4 string or a built-in shortcut (`EPSG:4839` ETRS89/LCC Germany, `EPSG:25832/25833` ETRS89/UTM). `proj4` (bundled, lazy-imported) reprojects each Carto tile to the cloud's CRS and places it at the cloud's actual coordinates. This is what NavVis IVION exports need (it stores georef in the **scan pose**, lifting local points to e.g. EPSG:4839; the LAS bakes that in → large `offset`).
- **`basemap.georeference` (manual-pin mode)** — for a cloud in LOCAL coords (small offset, near origin). Pins the cloud's local origin to a WGS84 `lat/lon` (+ optional `rotationDeg`/`metersPerUnit`); tiles are placed via a small-area equirectangular approximation.

Tile textures use `colorSpace = LinearSRGBColorSpace` (pass-through, matching potree's sRGB output) on a plane at the cloud's min-Z. The footprint is padded by `basemap.contextMeters` (default 250 m/side) so the map shows surrounding context, not just the cloud. The viewport builds it after load when `config.basemap?.crs` **or** `.georeference` is set (async — proj4 is lazy-loaded), flips `basemapAvailable`, and the Layers toggle drives `setVisible`.
