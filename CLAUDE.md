...
```
`TileBasemapManager` (`core/tile-basemap-manager.ts`) then lays **Carto Voyager raster XYZ tiles** (default; `tileUrl` overridable, attribution shown) on a ground plane at the cloud's min-Z. It builds tiles in a local ENU frame (small-area equirectangular: `lat/lon` = the cloud's local origin, `rotationDeg` = +Y heading from north, `metersPerUnit` default 1), then rotates the group to cloud heading. Tile textures use `colorSpace = LinearSRGBColorSpace` (pass-through, matching potree's sRGB output). The viewport builds it after load when `config.basemap?.georeference` is set, flips `basemapAvailable`, and the Layers toggle drives `setVisible`. An auto path from an embedded CRS (proj4) can be added later; the manual pin covers the common ungeoreferenced case.
