...
- **`addBox(box, name?)`**: Adds a `ClipBoxEntry`, creates a `Box3Helper` outline in the scene (black while deselected), applies clipping to all point clouds. Returns the entry. A selected box shows **all handle sets simultaneously** — move arrows + full 3-axis rotate rings + the 6 face-resize spheres — with **no mode switching**. Overlap conflicts are solved by size separation (compact arrows `setSize(0.55)` sit well inside the large rings `setSize(1.25)`; face spheres are pushed off the faces) plus **mutual drag exclusion**: when one `TransformControls` starts dragging, its `dragging-changed` handler synchronously disables the sibling before the sibling's own pointerdown listener runs (same-element listeners fire in registration order). Orientation is stored as a full `THREE.Quaternion` on the entry (`ClipBoxEntry.quaternion`). `setTransformMode`/`getTransformMode` are **deprecated no-ops** kept for API compatibility.
