...
- **`addBox(box, name?)`**: Adds a `ClipBoxEntry`, creates a `Box3Helper` outline in the scene (black while deselected), applies clipping to all point clouds. Returns the entry. When a box is selected, **all transform handles are shown at once** — translate arrows (`tcMove`), full **3-axis** rotation rings (`tcRotate`, `showX/Y/Z` all on), and the 6 axis-colored face-resize handles (`FaceHandleController`) — so there is no transform "mode" to switch. Orientation is stored as a full `THREE.Quaternion` on the entry (`ClipBoxEntry.quaternion`, replacing the old Z-only `rotationZ`).
...
- **Handle de-confliction**: the move arrows, rotate rings and face-resize spheres overlap spatially, and `TransformControls` listens on the canvas independently of the face-handle hit-test — so a press could grab two at once. `FaceHandleController.onHoverChange` fires when a resize sphere is hovered, and `ClipManager._setGizmosEnabled(false)` then disables the move/rotate gizmos (never mid-drag) so only the sphere is grabbable. The face spheres are also nudged slightly off the faces so they don't sit on the arrows/rings.
- **`setTransformMode(mode)`** *(deprecated, no-op)*: Transform handles are now all shown simultaneously, so there is no active mode. Kept only for API compatibility.
