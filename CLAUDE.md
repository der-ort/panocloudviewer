...
- **`update()`**: Called every frame via `SceneManager.addFrameCallback`. If bounds never arrived (e.g. the load promise rejected), `_deriveBoundsFromClouds()` self-heals them from the loaded octrees (tight box + offset). Throttled: renders 3D every 6th frame (~10 fps), overlay every 2nd frame. `_syncSize()` self-heals canvas dimensions each 3D render.
