...
Dark/light/system theme. Persists to `localStorage` (`pcv-theme` key). Applies `dark`/`light` class and `data-theme` attribute to `document.documentElement`. Exposes: `theme`, `resolvedTheme`, `setTheme()`, `toggleTheme()`. **The `theme` prop is controllable**: an effect syncs `defaultTheme` prop *changes* into state (skipping the first run so the persisted choice wins on mount), so an embedding site can drive dark/light from its own toggle by changing the `theme` prop; the in-viewer toggle still works between prop changes.
