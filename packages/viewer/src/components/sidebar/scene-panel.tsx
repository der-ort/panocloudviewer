...
import { CloudCog, Ruler, Trash2 } from "lucide-react";
...
export function ScenePanel() {
  ...
  return (
    <div className="flex flex-col h-full overflow-y-auto text-xs">
      ...
      {/* Sections (clipping) — Professional only; full controls live here now
          (the floating clip toolbar was removed). */}
      {isPro && (
        <div className="border-t border-[hsl(var(--border))]">
          <p className="text-[10px] font-semibold text-muted-foreground uppercase tracking-wide px-2 pt-2">{t.sections}</p>
          <ClipToolbar />
        </div>
      )}
    </div>
